inherited fmLocalizarPedidos: TfmLocalizarPedidos
  Left = 244
  Top = 122
  Caption = 'Localizar Pedidos'
  ClientHeight = 517
  ClientWidth = 585
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object lblCodigo: TLabel [0]
    Left = 8
    Top = 8
    Width = 41
    Height = 13
    Caption = 'NPedido'
    FocusControl = edtPedido
  end
  object Label1: TLabel [1]
    Left = 160
    Top = 8
    Width = 32
    Height = 13
    Caption = 'Cliente'
    FocusControl = edtNomeFantasia
  end
  object Label3: TLabel [2]
    Left = 347
    Top = 8
    Width = 46
    Height = 13
    Caption = 'Vendedor'
  end
  object Label2: TLabel [3]
    Left = 84
    Top = 8
    Width = 53
    Height = 13
    Caption = 'Nota Fiscal'
    FocusControl = edtNotaFiscal
  end
  object lblListandoPedidos: TLabel [4]
    Left = 8
    Top = 491
    Width = 59
    Height = 16
    Caption = 'Listando.'
    Font.Charset = ANSI_CHARSET
    Font.Color = clGreen
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    Visible = False
  end
  object Label4: TLabel [5]
    Left = 8
    Top = 52
    Width = 70
    Height = 13
    Caption = 'Ordem Compra'
    FocusControl = edtOrdemCompra
  end
  object Label5: TLabel [6]
    Left = 96
    Top = 52
    Width = 67
    Height = 13
    Caption = 'Data Previs'#227'o'
  end
  object edtPedido: TEdit [7]
    Left = 8
    Top = 24
    Width = 73
    Height = 21
    TabOrder = 0
    OnKeyPress = edtPedidoKeyPress
    OnKeyUp = edtPedidoKeyUp
  end
  object edtNomeFantasia: TEdit [8]
    Left = 160
    Top = 24
    Width = 185
    Height = 21
    TabOrder = 2
    OnKeyUp = edtNomeFantasiaKeyUp
  end
  object edtFuncionario: TDBLookupComboBox [9]
    Left = 347
    Top = 24
    Width = 230
    Height = 21
    KeyField = 'Codigo'
    ListField = 'Nome'
    ListSource = dtsFuncionarios
    TabOrder = 3
    OnCloseUp = edtFuncionarioCloseUp
  end
  object dbgLocalizarPedidos: TDBGrid [10]
    Left = 8
    Top = 98
    Width = 568
    Height = 344
    DataSource = dtsPedidos
    TabOrder = 6
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clBlack
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    OnCellClick = dbgLocalizarPedidosCellClick
    Columns = <
      item
        Expanded = False
        FieldName = 'Data'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DataPrevisao'
        Title.Caption = 'Previs'#227'o'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DataProducao'
        Title.Caption = 'Produ'#231#227'o'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'HoraProducao'
        Title.Caption = 'Hora Prod.'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NPedido'
        Width = 73
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Fatura'
        Title.Caption = 'Nota Fiscal'
        Width = 75
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Cliente'
        Width = 181
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Vendedor'
        Width = 182
        Visible = True
      end>
  end
  object btnCancelar: TBitBtn [11]
    Left = 485
    Top = 482
    Width = 90
    Height = 25
    Caption = 'Cancelar'
    TabOrder = 7
    OnClick = btnCancelarClick
    Glyph.Data = {
      
      ADADABABABA3A3A39B9B9BFF00FFFF00FFFF00FFFF00FFFF00FF}
    NumGlyphs = 2
  end
  object edtNotaFiscal: TEdit [12]
    Left = 84
    Top = 24
    Width = 73
    Height = 21
    TabOrder = 1
    OnKeyUp = edtNotaFiscalKeyUp
  end
  object rdgStatusPedido: TRadioGroup [13]
    Left = 8
    Top = 448
    Width = 568
    Height = 33
    BiDiMode = bdLeftToRight
    Caption = 'Status do Pedido'
    Columns = 4
    ItemIndex = 0
    Items.Strings = (
      'Geral'
      'Pendentes Produ'#231#227'o'
      'Pendentes Financeiro'
      'Finalizados')
    ParentBiDiMode = False
    TabOrder = 8
    OnClick = rdgStatusPedidoClick
  end
  object edtOrdemCompra: TEdit [14]
    Left = 8
    Top = 68
    Width = 81
    Height = 21
    TabOrder = 4
    OnKeyUp = edtOrdemCompraKeyUp
  end
  object edtDataPrevisao: TDateEdit [15]
    Left = 96
    Top = 68
    Width = 107
    Height = 21
    NumGlyphs = 2
    YearDigits = dyFour
    TabOrder = 5
    OnChange = edtDataPrevisaoChange
  end
  inherited popGrid: TPopupMenu
    Left = 472
    Top = 336
  end
  object tabPedidos: TZQuery
    Connection = fmMenu.zcnConexao
    Params = <>
    Left = 712
    Top = 8
  end
  object dtsPedidos: TDataSource
    AutoEdit = False
    DataSet = tabPedidos
    Left = 808
    Top = 8
  end
  object tabFuncionarios: TZQuery
    Connection = fmMenu.zcnConexao
    Params = <>
    Left = 712
    Top = 56
  end
  object dtsFuncionarios: TDataSource
    AutoEdit = False
    DataSet = tabFuncionarios
    Left = 808
    Top = 56
  end
end
