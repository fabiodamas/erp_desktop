inherited fmLocalizarFaturamento: TfmLocalizarFaturamento
  Left = 232
  Top = 153
  Caption = 'Localizar Faturamento'
  ClientHeight = 492
  ClientWidth = 589
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object lbl_codigo: TLabel [0]
    Left = 8
    Top = 8
    Width = 53
    Height = 13
    Caption = 'Nota Fiscal'
    FocusControl = edtFatura
  end
  object lblTipoConta: TLabel [1]
    Left = 104
    Top = 8
    Width = 32
    Height = 13
    Caption = 'Cliente'
    FocusControl = edtNomeFantasia
  end
  object Label3: TLabel [2]
    Left = 344
    Top = 8
    Width = 46
    Height = 13
    Caption = 'Vendedor'
  end
  object lblNotasEncontradas: TLabel [3]
    Left = 416
    Top = 0
    Width = 159
    Height = 13
    Alignment = taRightJustify
    AutoSize = False
    Caption = 'notas encontradas.'
    Font.Charset = ANSI_CHARSET
    Font.Color = clGreen
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Visible = False
  end
  object edtFatura: TEdit [4]
    Left = 8
    Top = 24
    Width = 89
    Height = 21
    TabOrder = 0
    OnKeyPress = edtFaturaKeyPress
    OnKeyUp = edtFaturaKeyUp
  end
  object edtNomeFantasia: TEdit [5]
    Left = 104
    Top = 24
    Width = 233
    Height = 21
    TabOrder = 1
    OnKeyUp = edtNomeFantasiaKeyUp
  end
  object edtFuncionario: TDBLookupComboBox [6]
    Left = 344
    Top = 24
    Width = 233
    Height = 21
    KeyField = 'Codigo'
    ListField = 'Nome'
    ListSource = dtsFuncionarios
    TabOrder = 2
    OnCloseUp = edtFuncionarioCloseUp
  end
  object dbgLocalizarFatura: TDBGrid [7]
    Left = 8
    Top = 56
    Width = 568
    Height = 392
    DataSource = dtsFaturamento
    TabOrder = 3
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clBlack
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    OnCellClick = dbgLocalizarFaturaCellClick
    Columns = <
      item
        Expanded = False
        FieldName = 'Data'
        Width = 71
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Fatura'
        Title.Caption = 'Nota Fiscal'
        Width = 84
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Vendedor'
        Width = 201
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NomeConta'
        Width = 216
        Visible = True
      end>
  end
  object btnCancelar: TBitBtn [8]
    Left = 485
    Top = 457
    Width = 90
    Height = 25
    Caption = 'Cancelar'
    TabOrder = 4
    OnClick = btnCancelarClick
    Glyph.Data = {
      
      ADADABABABA3A3A39B9B9BFF00FFFF00FFFF00FFFF00FFFF00FF}
    NumGlyphs = 2
  end
  object rdgStatusNota: TRadioGroup [9]
    Left = 9
    Top = 450
    Width = 287
    Height = 36
    Caption = 'Status Nota'
    Columns = 3
    ItemIndex = 0
    Items.Strings = (
      'Geral'
      'Normal'
      'Cancelada')
    TabOrder = 5
    OnClick = rdgStatusNotaClick
  end
  object tabFaturamento: TZQuery
    Connection = fmMenu.zcnConexao
    Params = <>
    Left = 712
    Top = 8
  end
  object dtsFaturamento: TDataSource
    AutoEdit = False
    DataSet = tabFaturamento
    Left = 808
    Top = 8
  end
  object tabFuncionarios: TZQuery
    Connection = fmMenu.zcnConexao
    Params = <>
    Left = 712
    Top = 56
  end
  object dtsFuncionarios: TDataSource
    AutoEdit = False
    DataSet = tabFuncionarios
    Left = 808
    Top = 56
  end
end
