inherited fmLocalizarPedidosCompra: TfmLocalizarPedidosCompra
  Left = 244
  Top = 122
  Caption = 'Localizar Pedidos de Compra'
  ClientHeight = 492
  ClientWidth = 585
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object lbl_codigo: TLabel [0]
    Left = 8
    Top = 8
    Width = 41
    Height = 13
    Caption = 'NPedido'
    FocusControl = edtPedido
  end
  object Label1: TLabel [1]
    Left = 160
    Top = 8
    Width = 54
    Height = 13
    Caption = 'Fornecedor'
    FocusControl = edtFornecedor
  end
  object Label3: TLabel [2]
    Left = 347
    Top = 8
    Width = 55
    Height = 13
    Caption = 'Funcion'#225'rio'
  end
  object Label2: TLabel [3]
    Left = 84
    Top = 8
    Width = 53
    Height = 13
    Caption = 'Nota Fiscal'
    FocusControl = edtNotaFiscal
  end
  object edtPedido: TEdit [4]
    Left = 8
    Top = 24
    Width = 73
    Height = 21
    TabOrder = 0
    OnKeyPress = edtPedidoKeyPress
    OnKeyUp = edtPedidoKeyUp
  end
  object edtFornecedor: TEdit [5]
    Left = 160
    Top = 24
    Width = 185
    Height = 21
    TabOrder = 2
    OnKeyUp = edtFornecedorKeyUp
  end
  object edtFuncionario: TDBLookupComboBox [6]
    Left = 347
    Top = 24
    Width = 230
    Height = 21
    KeyField = 'Codigo'
    ListField = 'Nome'
    ListSource = dtsFuncionarios
    TabOrder = 3
    OnCloseUp = edtFuncionarioCloseUp
  end
  object dbgLocalizarPedidos: TDBGrid [7]
    Left = 8
    Top = 56
    Width = 568
    Height = 392
    DataSource = dtsPedidos
    TabOrder = 4
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clBlack
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    OnCellClick = dbgLocalizarPedidosCellClick
    Columns = <
      item
        Expanded = False
        FieldName = 'Data'
        Width = 74
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Pedido'
        Width = 73
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NotaFiscal'
        Title.Caption = 'Nota Fiscal'
        Width = 75
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Fornecedor'
        Width = 181
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Vendedor'
        Width = 182
        Visible = True
      end>
  end
  object btnCancelar: TBitBtn [8]
    Left = 485
    Top = 457
    Width = 90
    Height = 25
    Caption = 'Cancelar'
    TabOrder = 5
    OnClick = btnCancelarClick
    Glyph.Data = {
      
      ADADABABABA3A3A39B9B9BFF00FFFF00FFFF00FFFF00FFFF00FF}
    NumGlyphs = 2
  end
  object edtNotaFiscal: TEdit [9]
    Left = 84
    Top = 24
    Width = 73
    Height = 21
    TabOrder = 1
    OnKeyUp = edtNotaFiscalKeyUp
  end
  object rdgStatusPedido: TRadioGroup [10]
    Left = 8
    Top = 451
    Width = 249
    Height = 36
    Caption = 'Status do Pedido'
    Columns = 3
    ItemIndex = 0
    Items.Strings = (
      'Geral'
      'Em Aberto'
      'Recebidos')
    TabOrder = 6
    OnClick = rdgStatusPedidoClick
  end
  object tabPedidos: TZQuery
    Connection = fmMenu.zcnConexao
    Params = <>
    Left = 624
    Top = 16
  end
  object dtsPedidos: TDataSource
    AutoEdit = False
    DataSet = tabPedidos
    Left = 720
    Top = 16
  end
  object tabFuncionarios: TZQuery
    Connection = fmMenu.zcnConexao
    Params = <>
    Left = 624
    Top = 64
  end
  object dtsFuncionarios: TDataSource
    AutoEdit = False
    DataSet = tabFuncionarios
    Left = 720
    Top = 64
  end
end
